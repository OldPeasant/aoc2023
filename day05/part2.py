...
def calc_seeds(seeds_input):
    all_seeds = []
    for ix in range(0, len(seeds_input), 2):
        for s in range(seeds_input[ix], seeds_input[ix] + seeds_input[ix + 1]):
            all_seeds.append(s)
    return all_seeds
...
with open(sys.argv[1]) as f:
    ...
    locations = []
    all_seeds = calc_seeds(seeds_input)
    print("Number of seeds: " + str(len(all_seeds)))
    ix = 0
    for s in calc_seeds(seeds_input):
        ix += 1
        print("Examine seed no " + str(ix) + ": " + str(s))
